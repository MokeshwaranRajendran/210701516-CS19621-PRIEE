...
Sub Activity_Create(FirstTime As Boolean)
	...
	Table1.SetHeader(Array As String("Name", "Population", "Flood","" ))
	
	
	Main.cursor1 = Main.sql1.ExecQuery("SELECT * FROM tblUrban1")
For i = 0 To Main.cursor1.RowCount - 1
Main.cursor1.Position = i
    Table1.AddRow(Array As String(Main.cursor1.getString("uName"), Main.cursor1.getString("uPopulation"), Main.cursor1.getString("uFlood")))
	Next
	Table1.SetColumnsWidths(Array As Int(50%x, 30%x, 20%x))
